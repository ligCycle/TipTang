  ...
  return (
    <div className="mx-auto max-w-2xl space-y-8">
      ...
      <section className="card rounded-3xl p-8 text-center">
        <div className="mx-auto flex h-24 w-24 items-center justify-center rounded-full bg-gradient-to-br from-brand-400 to-brand-600 text-4xl font-black text-white shadow-lg">
          {initial}
        </div>
        <h1 className="mt-4 text-2xl font-extrabold text-brand-900">
          {creator.displayName}
        </h1>
        <p className="text-sm font-medium text-brand-600">@{creator.username}</p>
        {creator.bio && (
          <p className="mx-auto mt-3 max-w-md text-brand-900/70">{creator.bio}</p>
        )}
      </section>
